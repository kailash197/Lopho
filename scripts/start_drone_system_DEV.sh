...
docker build -t humble_machine:$date_tag /home/$(whoami)/lopho_project/.devcontainer/

# Display a message indicating the start of Docker run
echo "Docker Run"
docker run -itd --privileged --name humble_container --user vscode --network host --ipc host -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=$DISPLAY -v $PWD:/lopho_project humble_machine:20240301
